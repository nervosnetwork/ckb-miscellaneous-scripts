...
#define         MolBuilder_OrScripts_init(b)                    mol_builder_initialize_with_capacity(b, 1024, 64)
...
#define         MolBuilder_OrWitnesses_init(b)                  mol_builder_initialize_with_capacity(b, 64, 64)
...
mol_errno MolReader_OrWitnesses_verify (const mol_seg_t *input, bool compatible) {
    if (input->size < MOL_NUM_T_SIZE) {
        return MOL_ERR_HEADER;
    }
    uint8_t *ptr = input->ptr;
