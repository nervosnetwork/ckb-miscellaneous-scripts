...
/**
 * Note: there is no prefilled data for RSA, it's only be used for secp256k1.
 * Always succeed.
 * @param data
 * @param len
 * @return
...
validate_rsa_sighash_all(uint8_t *output_public_key_hash) {
...
__attribute__((visibility("default"))) int validate_simple() {
    ...
    ret = validate_rsa_sighash_all(public_key_hash);
    if (ret != CKB_SUCCESS) {
        return ret;
    }
